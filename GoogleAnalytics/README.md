...
```bash
EventBridgeTest$ sam local invoke ScheduledEventLogger --event events/event-cloudwatch-event.json
```
...
```bash
EventBridgeTest$ sam logs -n ScheduledEventLogger --stack-name sam-app --tail
```
...
```bash
EventBridgeTest$ dotnet test test/EventBridgeTest.Tests/EventBridgeTest.Tests.csproj 
```
...
```bash
aws cloudformation delete-stack --stack-name EventBridgeTest
```
